...
func strStr(haystack string, needle string) int {
	// Mengiterasi setiap karakter dalam haystack
	for i := 0; i <= len(haystack)-len(needle); i++ {
		// Memeriksa kemungkinan kemunculan needle pada indeks i dalam haystack
...
func searchInsert(nums []int, target int) int {
	// Menentukan batas bawah dan batas atas untuk pencarian biner
	left, right := 0, len(nums)-1

	// Pencarian biner untuk menemukan indeks di mana target harus dimasukkan
	for left <= right {
		mid := (left + right) / 2
		if nums[mid] == target {
			return mid
	...
	// Jika target lebih besar dari elemen terakhir, masukkan ke indeks terakhir+1
	return left
}

// soal no 23 = easy
func lengthOfLastWord(s string) int {
	// Menghapus spasi ekstra di bagian akhir dan awal string
	s = strings.TrimSpace(s)

	// Mencari posisi spasi terakhir
	lastSpaceIndex := strings.LastIndex(s, " ")

	// Jika tidak ada spasi, maka panjang kata terakhir adalah panjang string s
	if lastSpaceIndex == -1 {
		return len(s)
	}

	// Jika ada spasi, maka panjang kata terakhir adalah panjang dari substring setelah spasi terakhir
...
func plusOne(digits []int) []int {
	...
	// Menambahkan 1 pada angka terakhir
	digits[n-1]++

	// Melakukan penanganan carry
	for i := n - 1; i > 0; i-- {
		if digits[i] == 10 {
			digits[i] = 0
			digits[i-1]++
	...
	// Jika angka pertama adalah 10, maka perlu menambahkan digit 1 pada awal slice
	if digits[0] == 10 {
		digits[0] = 0
...
func addBinary(a string, b string) string {
	// Menginisialisasi variabel untuk menyimpan hasil penjumlahan biner
	result := ""

	// Menentukan panjang string a dan b
	lenA := len(a)
	lenB := len(b)

	// Menentukan batas atas untuk loop
	maxLen := lenA
	if lenB > lenA {
		maxLen = lenB
	}

	// Inisialisasi variabel untuk carry
	carry := 0

	// Melakukan penjumlahan bit per bit dari belakang ke depan
	for i := 0; i < maxLen; i++ {
		// Mengambil bit dari string a dan b dari belakang
		bitA := 0
		bitB := 0

		if i < lenA {
			bitA = int(a[lenA-1-i] - '0')
		}
		if i < lenB {
			bitB = int(b[lenB-1-i] - '0')
		}

		// Melakukan penjumlahan bit dan carry
		sum := bitA + bitB + carry

		// Menghitung bit untuk hasil penjumlahan saat ini
		currBit := sum % 2

		// Menghitung carry untuk penjumlahan berikutnya
		carry = sum / 2

		// Menambahkan bit hasil penjumlahan saat ini ke awal string result
		result = fmt.Sprintf("%d%s", currBit, result)
	}

	// Jika masih ada carry pada akhir operasi penjumlahan, tambahkan ke hasil akhir
	if carry > 0 {
...
func mySqrt(x int) int {
	// Kasus khusus untuk 0 dan 1
	if x <= 1 {
		return x
	}

	// Metode Binary Search untuk mencari akar kuadrat
	left, right := 1, x
	var ans int
...
func climbStairs(n int) int {
	// Kasus khusus untuk n = 0 dan n = 1
	if n <= 1 {
		return 1
	}

	// Menggunakan metode Dynamic Programming untuk menghitung jumlah cara mendaki tangga
	dp := make([]int, n+1)
	dp[0], dp[1] = 1, 1
...
func deleteDuplicates(head *ListNode) *ListNode {
	...
	// Mengecek setiap node untuk menghapus duplikat
	current := head
...
func singleNumber(nums []int) int {
	// Inisialisasi variabel untuk menyimpan hasil XOR dari seluruh elemen nums
	result := 0

	// Melakukan operasi XOR pada setiap elemen nums
	for _, num := range nums {
		result ^= num
	}
...
func generate(numRows int) [][]int {
	...
	// Inisialisasi segitiga Pascal dengan elemen 1 di baris pertama
	pascalTriangle := [][]int{{1}}

	// Mengisi segitiga Pascal dengan menghitung nilai setiap elemen berdasarkan baris dan kolom
	for i := 1; i < numRows; i++ {
...
func main() {
	...
	insertIndex := searchInsert(nums22, target)
	fmt.Println(insertIndex)

	fmt.Print("No. 23 : ")
	s23 := "Hello World"
	length := lengthOfLastWord(s23)
	fmt.Println(length)

	fmt.Print("No. 24 : ")
	digits24 := []int{1, 2, 3}
	result24 := plusOne(digits24)
	fmt.Println(result24)
	...
	fmt.Print("No. 27 : ")
	n27 := 4
	ways := climbStairs(n27)
	fmt.Println(ways)

	fmt.Print("No. 28 : ")
	// Contoh linked list: 1 -> 1 -> 2 -> 3 -> 3
	list := &ListNode{Val: 1}
	...
	// Menghapus duplikat
	result28 := deleteDuplicates(list)

	fmt.Println("Linked List setelah menghapus duplikat:")
	printList(result28)

	fmt.Print("No. 29 : ")
	nums29 := []int{4, 1, 2, 1, 2}
	single := singleNumber(nums29)
	fmt.Println(single)
